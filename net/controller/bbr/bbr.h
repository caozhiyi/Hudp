...
#define CYCLE_LEN    8    /* number of phases in a pacing gain cycle  发送速度增益因子周期内的阶段数*/
    ...
    private:
        ...
        uint64_t probe_rtt_done_stamp;      /* end time for BBR_PROBE_RTT mode ， BBR_PROBE_RTT结束时间戳*/
        struct minmax bw;                   /* Max recent delivery rate in pkts/uS << 24 本次bw */
        uint32_t rtt_cnt;                   /* count of packet-timed rounds elapsed */
        uint64_t next_rtt_delivered;        /* scb->tx.delivered at end of round, 在回合结束时发送的数据量 */
        uint64_t cycle_mstamp;              /* time of this cycle phase start 本轮周期开始时间*/
        ...
        uint64_t lt_bw;                     /* LT est delivery rate in pkts/uS << 24 */
    ...
    private:
        ...
        uint32_t bbr_bw() { return lt_use_bw ? (uint32_t)lt_bw : bbr_max_bw(); }
